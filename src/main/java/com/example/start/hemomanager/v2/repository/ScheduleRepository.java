...
//    @Query("SELECT new com.example.start.hemomanager.v2.domain.Schedule(s.uuid, s.scheduledDonor, s.hemocenter, s.scheduleHemocenter) FROM Schedule s WHERE hemocenter_uuid = ?1")
//    @Query("SELECT s FROM Schedule s WHERE s.hemocenter.uuid = :uuid")
    List<Schedule> findAllByHemocenter_Uuid(Integer uuid);
//    @Query("SELECT new com.example.start.hemomanager.v2.domain.Schedule(s.uuid, s.scheduledDonor, s.hemocenter, s.scheduleHemocenter) FROM Schedule s WHERE s.hemocenter.uuid = ?1")
